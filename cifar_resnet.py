...
class LearnableAlpha(nn.Module):
    def __init__(self, out_channel):
        ...
        self.alphas = nn.Parameter(torch.ones(1, out_channel, 1, 1), requires_grad=True)
...
class BasicBlock_IN(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        # print(self.alpha1.alphas.shape, self.alpha2.alphas.shape)
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.alpha = LearnableAlpha(_outputs[0])
...
class ResNet_IN(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        out = F.avg_pool2d(out, 4 if self.args.stride == 1 else 2)
        out = out.view(out.size(0), -1)
        out = self.linear(out)
...
class WideBasicBlock(nn.Module):
    def __init__(self, in_planes, out_planes, stride, dropRate=0.0):
        ...
        self.alpha1 = LearnableAlpha(in_planes)
        ...
        self.conv2 = nn.Conv2d(out_planes, out_planes, kernel_size=3, stride=1,
                               padding=1, bias=False)
        self.alpha2 = LearnableAlpha(out_planes)
...
class WideResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.bn1(out)
        out = F.avg_pool2d(out, 8 if self.args.stride == 1 else 4)
